...
def test_emb(
    ...
    embedding = F.normalize(embedding, dim=1) #F.normalize(embedding, dim=1).cpu().numpy()
    pdist = torch.mm(embedding, embedding.t()).cpu().numpy() #np.dot(embedding, embedding.T)
    gt = id_labels.expand(n, n).eq(id_labels.expand(n, n).t()).numpy()
    print('Done computing')
    print(np.sort(pdist[0][gt[0] == True])[::-1][:60])
    print(np.sort(pdist[0][gt[0] == False])[::-1][:60])
