...
def xyxy2stdxywh(xyxy, width,height) :
    xywh = xyxy2xywh(xyxy)  # / height
    xywh[:, 0] /= width
    xywh[:, 1] /= height
    xywh[:, 2] /= width
    xywh[:, 3] /= height
...
def bbox_iou_numpy(pred_box, gt_box):
...
# find position in a,b that have same value 
def list_mapping(a,b) :
    a_dict = { idx : val for idx,val in enumerate(a)}
    b_dict = { idx : val for idx,val in enumerate(b)}
    a_dict = sorted(a_dict.items(), key=lambda x: x[1])
    b_dict = sorted(b_dict.items(), key=lambda x: x[1])
    mapping = []
    for a_item in a_dict :
        for b_item_idx, b_item in enumerate(b_dict) :
            if a_item[1] <= b_item[1] :
                if a_item[1] == b_item[1] :
                    mapping.append([a_item[0],b_item[0]])
                    b_dict = b_dict[b_item_idx + 1:]
                else:
                    b_dict = b_dict[b_item_idx:]
                break
    return mapping
